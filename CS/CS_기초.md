...
14. 멀티프로세스와 멀티스레드의 차이점을 설명해주세요.
    - 멀티프로세스
        ...
        - 각 프로세스별로 독립된 메모리 공간을 갖기 때문에 문맥교환을 수행할 때 이전 프로세스의 데이터를 공유하지 않으므로 캐시 메모리 내 데이터가 무효화됨에 따른 성능 저하가 발생할 수 있다.
        - 하나의 프로세스에 문제가 발생하여도 전체 애플리케이션에 영향을 주지 않는다.
    - 멀티스레드
        - 하나의 프로세스 내에서 여러 스레드가 작업을 수행하는 방식
        - 스레드가 생성될 때마다 메모리 영역 중 Stack을 제외한 나머지 영역(Code, Data, Heap)을 공유하므로 문맥교환을 수행할 때 이전 스레드의 캐싱된 데이터를 사용할 수 있어 성능 이점이 있다.
        - 하나의 스레드에 문제가 발생하면 해당 프로세스에 영향을 줄 수 있다.
15. 데몬이란 무엇인지 설명해주세요.
    - 사용자와 상호작용하지 않고 특정 요청이 오면 작업을 수행하는 백그라운드 프로세스
16. 문맥 교환 과정에 대해 설명해주세요.
    - 현재 실행 중인 프로세스/스레드가 CPU를 사용 중일 때 CPU 스케줄링 알고리즘에 의해 다른 프로세스/스레드에게 CPU를 양보해야 하는 경우 현재 프로세스의 상태를 PCB에 저장하고 다음 실행될 프로세스의 정보가 담긴 PCB를 읽어서 실행을 시작한다.
...
21. 교착상태란 무엇인지, 교착상태가 발생할 수 있는 조건을 모두 설명해주세요.
    - 여러 프로세스가 서로 보유 중인 자원을 사용할 수 있을 때까지 대기하는 상태
...
23. 요구 페이징 처리 순서를 서술해주세요.
    ...
    5. 다시 1번을 수행한다.
25. Thrashing이란 무엇인지 설명하고 이를 해결하기 위한 기법을 하나 이상 설명해주세요.
    - Thrashing
    ...
    - Page fault frequency
        - 페이지 폴트율에 상한선과 하한선을 정하고 그 내부 범위 안에서 프레임을 할당하는 방식
...
26. 유닉스 파일 시스템에 대해 설명해주세요.
    - inode 기반 파일시스템
        - 파일의 크기, 권한, 생성 시간 등과 같은 메타데이터와 실제 데이터(또는 주소)를 저장하는 블록으로 이루어진 자료구조
            - 파일 생성 시 고유번호를 부여하고 Inode block이 생성되어 메타데이터를 저장한다.
            - 파일 접근 시 [file-name]:[inode-number]으로 매칭하여 접근한다.
                ```bash
                $echo "file test" > test
                $ls -li
                29476 -rw-r--r-- 2 eljoe eljoe   15 Oct 18 20:37 test
                ```
        - 실제 데이터(또는 주소)는 Direct block에 저장한다.
        ...
        - 사용 이유
            - 메타데이터를 하나의 Inode block으로 관리하기 때문에 파일 정보를 빠르게 참조할 수 있다.
...
26. 컴퓨터의 부팅 과정을 서술해주세요.
    ...
    3. 테스트가 완료되면 BIOS는 사용자가 설정한 부팅 순서에 따라 보조기억장치를 선택하고 첫 섹터(0번 섹터)인 MBR(Master Boot Record)에 저장된 초기 부트로더를 실행한다.
